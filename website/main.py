...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        if predlabel == 'software_developer':
            ...
            predid = 9
        try:
            sql = "SELECT * FROM job JOIN company ON company.company_id = job.comp_id JOIN category ON category.category_id = %s"
            data = (predid)
            cursor.execute(sql, data)
            rows = cursor.fetchall()
            #resp = jsonify(rows)
            #resp.status_code = 200
            return render_template('rec_job.html', rows=rows)
        except Exception as e:
            print(e)
        finally:
            cursor.close()
